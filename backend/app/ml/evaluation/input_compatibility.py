...
def run_experiment():
    ...
    groups = [
        ("0-100", lambda l: l <= 100),
        ("100-250", lambda l: 100 < l <= 250),
        ("250-500", lambda l: 250 < l <= 500),
        ("500-1000", lambda l: 500 < l <= 1000),
        ("1000-2500", lambda l: 1000 < l <= 2500),
        ("2500+", lambda l: l > 2500),
    ]

    results = {}
    for name, condition in groups:
        mask = condition(tokens_len)
        ...
        results[name] = {
            ...
            "accuracy": acc,
            "macro_f1": f1,
            "avg_vocabulary_coverage": float(np.mean(cov_g)),
            "average_probability_distance_from_0_5": float(np.mean(probs_g - 0.5)),
            "LOW_SAMPLE_WARNING": count < 30
        }

    out_file = r"C:\Users\HP-VICTUS\Downloads\major_project\backend\app\ml\artifacts\experiments\input_compatibility.json"
