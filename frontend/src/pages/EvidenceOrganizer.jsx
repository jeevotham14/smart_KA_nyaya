...
export default function EvidenceOrganizer() {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl section-fade visible min-h-screen">
      <div className="mb-10">
        <h1 className="text-3xl font-bold text-navy-900 dark:text-white mb-2 flex items-center gap-3">
          <svg className="w-8 h-8 text-legalGold" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 002-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
          </svg>
          Evidence Organizer
        </h1>
        <p className="text-slate-600 dark:text-slate-400 text-lg">AI-powered evidence classification and management for Case #{caseId}</p>
        {error && <p className="text-sm text-alertRed mt-2">{error}</p>}
      </div>

      <div className="mb-10 animate-fade-in-up" style={{ animationDelay: '100ms' }}>
        <FileUploadZone onUpload={handleUpload} />
        {isUploading && (
          <div className="mt-4 flex items-center justify-center text-sm text-navy-900 dark:text-white font-medium p-4 rounded-2xl border border-legalGold/30 bg-white dark:bg-navy-900 shadow-sm glass-panel">
            <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-legalGold" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
            <div key={i} className="h-[320px] p-6 skeleton rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-navy-900 shadow-sm glass-panel"></div>
          ))}
        </div>
      ) : (
